		...
		for x in range(len(df.index)):
			try:
				...
				if math.isnan(ABSX) or math.isnan(ABSY):#非数字跳过
					continue
				bdo9= coordinate_util.bd09togcj02(ABSX, ABSY)
				wgs84= coordinate_util.gcj02towgs84(bdo9[0], bdo9[1])
				jd84=wgs84[0]
				wd84=wgs84[1]
				# print(jd84+'-----'+wd84)
				...
				lon = zb_arr[0]  # 经度坐标
				lat = zb_arr[1]
				df.set_value(x,'lon',lon)
				df.set_value(x,'lat',lat)
				time.sleep(0.04)
...
if __name__ == '__main__':
	filename='C:\\Users\\admin\\Desktop\\按季度分别落图.csv'
	change_zb(filename)